...
int main() 
{
	...
	for(i=0; i<np; i++) pthread_create(&prod[i], NULL, producer, NULL);
	for(i=0; i<nc; i++) pthread_create(&cons[i], NULL, consumer, NULL);
	for(i=0; i<np; i++) pthread_join(prod[i], NULL);
	for(i=0; i<nc; i++) pthread_join(cons[i], NULL);
